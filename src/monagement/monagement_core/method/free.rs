...
impl MonagementCore {
    ...
    pub(crate) fn free_unchecked(&mut self, allocated: &Allocated) -> Result<(), String> {
        unsafe {
            let link = allocated.link;
            // handler
            let allocated_node_index;
            ...
            let mut new_end = None;
            {
                ...
                if let Some(back_node_link) = allocated_node_back {
                    ...
                    if let NodeStatus::Free(fl, sl, sl_idx) = &back_node.status {
                        ...
                        // clean node
                        self.clean_free_node_in_fl_sl_unchecked(
                            *fl as usize,
                            *sl as usize,
                            sl_idx.0,
                        )
                ...
                let second_level = first_level.sl_list.get_unchecked_mut(sl as usize);
    ...
    pub fn clean_free_node_in_fl_sl_unchecked(
        &mut self,
        fl: usize,
        sl: usize,
        sl_idx: usize,
        ...
        unsafe {
            // get first level and second level
            let first_level = self.fl_list.get_unchecked_mut(fl);

            let second_level = first_level.sl_list.get_unchecked_mut(sl);

            // update
            // // second level
            // // // link list
            let second_level_link = second_level
                .link_list
                .get_unchecked(sl_idx)
                .as_ref()
                .unwrap_unchecked();
