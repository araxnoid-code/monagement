...
impl MonagementCore {
    ...
    pub fn allocate_unchecke(&mut self, size: NonZeroU64) -> Result<Allocated, String> {
        unsafe {
            let target = size.get();
            let (fl_target, sl_target) = self.get_fl_sl(target);
            if target > self.max_size {
            ...
            'main: loop {
                ...
                let first_level = self.fl_list.get_unchecked_mut(fl_idx as usize);
                ...
                loop {
                    ...
                    let second_level = first_level.sl_list.get_unchecked_mut(sl_idx as usize);
                        ...
                        (second_level.head_link, second_level.end_link)
                    {
                        ...
                        loop {
                            // get first second level linked list
                            let second_link_list = second_level
                                .link_list
                                .get_unchecked(second_link_list_idx)
                                .as_ref()
                                .unwrap_unchecked();
                            let link_idx = second_link_list.node_link;
                            ...
                            if rest == 0 {
                                ...
                                // update front_node
                                if let Some(front_node_idx) = front_node_link {
                                    if let Some(front_node) =
                                        self.linked_list.get_unchecked_mut(front_node_idx)
                                    {
                                        front_node.back = Some(used_node_idx);
                                    };
                                }
            ...
            if let Some(link) = allocated_link_handler {
                    ...
                    (free_node_idx, new_front_link, new_size, new_start)
                {
                    ...
                    // new location
                    let (fl, sl) = get_fl_sl(n_size, 0, 0, self.minimum_size_raw);
                    let first_level = self.fl_list.get_unchecked_mut(fl as usize);

                    let second_level = first_level.sl_list.get_mut(sl as usize).unwrap_unchecked();
                    ...
                    let (sl_idx, push_handler) =
                        if let Some(idx) = second_level.free_link_list.pop() {
                            (idx, false)
                        } else {
                            (second_level.link_list.len(), true)
                        };
